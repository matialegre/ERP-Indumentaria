  ...
  mainWin.webContents.on('did-finish-load', () => {
    ...
    mainWin.webContents.executeJavaScript(`
      (function() {
        ...
        localStorage.setItem('erp_company_id', '6');
        ...
        return fetch('/api/v1/auth/me', { headers: { 'Authorization': 'Bearer ' + token } })
          .then(r => r.json())
          .then(u => {
            if (u.company_id && u.company_id !== 6) {
...
// Sirve el frontend buildeado desde electron-montagne/frontend-dist/
// Las llamadas API irán al servidor real (configurado en erp_server_url).
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    try {
      ...
      createMainWindow(localUrl, serverUrl); // pasa serverUrl para inyectarlo en localStorage
    } catch (localErr) {

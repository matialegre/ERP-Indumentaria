...
def alertas_reposicion(
    ...
    """Alertas de reposición: pedidos enviados con facturación incompleta."""
    company_id = current_user.company_id if current_user.role != UserRole.SUPERADMIN else None

    q = db.query(PurchaseOrder).filter(PurchaseOrder.status == PurchaseOrderStatus.ENVIADO)
    if company_id:
...
def accept_difference(
    ...
    if order.status not in [PurchaseOrderStatus.ENVIADO, PurchaseOrderStatus.RECIBIDO]:
        raise HTTPException(status_code=400, detail="Solo se puede aceptar diferencia en estado ENVIADO o RECIBIDO")
...
def reopen_order(
    ...
    db.commit()
    return {"ok": True, "message": "Nota reabierta a estado ENVIADO."}
...
def vista_integrada(
    ...
    if status:
        ...
        q = q.filter(PurchaseOrder.status.in_([
            PurchaseOrderStatus.ENVIADO, PurchaseOrderStatus.RECIBIDO
        ]))
    if local_id:
...
def create_purchase_order(
    ...
    try:
        order_type = PurchaseOrderType(body.type)
    except ValueError:
    ...
    if body.number:
        number = body.number
        prefix = body.prefix or provider.order_prefix or provider.name[:8].upper().strip()
    else:
        number, prefix = _auto_number(db, provider, company_id)

    total_ordered = sum(
        float(it.unit_cost or 0) * it.quantity_ordered for it in body.items
    ) or None
...
def send_purchase_order(
    ...
    """Marca la nota de pedido como ENVIADO (BORRADOR → ENVIADO)."""
    order = db.query(PurchaseOrder).filter(PurchaseOrder.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Nota de pedido no encontrada")
    if order.status != PurchaseOrderStatus.BORRADOR:
        raise HTTPException(status_code=400, detail="Solo se puede enviar una nota de pedido en BORRADOR")
    if not order.items:
        raise HTTPException(status_code=400, detail="No se puede enviar una nota de pedido sin ítems")
...
def receive_purchase_order(
    ...
    """Marca la nota de pedido como RECIBIDO (ENVIADO → RECIBIDO) y actualiza total_received."""
    order = db.query(PurchaseOrder).filter(PurchaseOrder.id == order_id).first()
    if not order:
        raise HTTPException(status_code=404, detail="Nota de pedido no encontrada")
    if order.status != PurchaseOrderStatus.ENVIADO:
        raise HTTPException(status_code=400, detail="Solo se puede recibir una nota de pedido en ENVIADO")
